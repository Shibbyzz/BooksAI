...
export default function BookDetailPage() {
  ...
  const exportToTxt = async () => {
    ...
    try {
      // Show loading state
      toast.loading(t('exportingBook'))
      
      // Fetch complete book content
      ...
      // Format content as TXT
      let txtContent = ''
      
      // Add title
      txtContent += `${bookContent.title}\n`
      txtContent += '='.repeat(bookContent.title.length) + '\n\n'
      
      // Add back cover if available
      if (bookContent.backCover) {
        txtContent += 'BACK COVER\n'
        txtContent += '-'.repeat(10) + '\n'
        txtContent += bookContent.backCover + '\n\n'
      }
      
      // Add chapters
      bookContent.chapters.forEach((chapter: any, index: number) => {
        // Chapter header
        txtContent += `CHAPTER ${chapter.chapterNumber}${chapter.title ? `: ${chapter.title}` : ''}\n`
        ...
        if (chapter.content) {
          ...
          txtContent += '[Chapter content not available]\n\n'
        }
        
        // Add spacing between chapters (except for the last one)
      ...
      txtContent += '\n' + '='.repeat(50) + '\n'
      txtContent += 'BOOK INFORMATION\n'
      txtContent += '='.repeat(50) + '\n'
      txtContent += `Generated by: BooksAI\n`
      txtContent += `Export Date: ${new Date().toLocaleDateString()}\n`
      txtContent += `Total Chapters: ${bookContent.chapters.length}\n`
      const totalWords = bookContent.chapters.reduce((sum: number, chapter: any) => sum + (chapter.wordCount || 0), 0)
      if (totalWords > 0) {
        txtContent += `Total Words: ${totalWords.toLocaleString()}\n`
      }
      ...
      const sanitizedTitle = bookContent.title.replace(/[^a-zA-Z0-9\-_\s]/g, '').replace(/\s+/g, '_')
      link.download = `${sanitizedTitle}_by_BooksAI.txt`
      ...
      toast.dismiss()
      toast.success(t('bookExportedSuccessfully'))
